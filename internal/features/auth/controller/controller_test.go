...
func TestGetUser_NotFoundAndSuccess(t *testing.T) {
    ...
    app.Get("/auth/user/:id", ctrl.GetUser)
    r, _ := http.NewRequest("GET", "/auth/user/abc", nil)
    resp, err := app.Test(r)
    ...
    app2 := fiber.New()
    app2.Get("/auth/user/:id", ctrl2.GetUser)
    r2, _ := http.NewRequest("GET", "/auth/user/507f1f77bcf86cd799439011", nil)
    resp2, err := app2.Test(r2)
    if err != nil { t.Fatalf("request failed: %v", err) }
    if resp2.StatusCode != 200 { t.Fatalf("expected 200 got %d", resp2.StatusCode) }
}

func TestSignInUser_InvalidPayloadAndAuth(t *testing.T) {
    app := fiber.New()

    // Body parser error -> invalid JSON
    ctrl := &AuthController{Query: &mockAuthQuery{}}
    app.Post("/auth/signin", ctrl.SignInUser)
    r, _ := http.NewRequest("POST", "/auth/signin", bytes.NewReader([]byte("invalid")))
    ...
    // Incorrect password
    pw, _ := bcrypt.GenerateFromPassword([]byte("correct.pass321"), bcrypt.DefaultCost)
    ...
    app2 := fiber.New()
    app2.Post("/auth/signin", ctrl2.SignInUser)
    body := bytes.NewReader([]byte(`{"email":"a@b.com","password":"wrong.pass123"}`))
    r2, _ := http.NewRequest("POST", "/auth/signin", body)
    ...
    app3 := fiber.New()
    app3.Post("/auth/signin", ctrl3.SignInUser)
    body3 := bytes.NewReader([]byte(`{"email":"a@b.com","password":"correct.pass321"}`))
    r3, _ := http.NewRequest("POST", "/auth/signin", body3)
...
func TestSignUpUser_ValidationAndCreate(t *testing.T) {
    ...
    app.Post("/auth/signup", ctrl.SignUpUser)
    r, _ := http.NewRequest("POST", "/auth/signup", bytes.NewReader([]byte("invalid")))
    r.Header.Set("Content-Type", "application/json")
    resp, err := app.Test(r)
    if err != nil { t.Fatalf("request failed: %v", err) }
    ...
    app2 := fiber.New()
    app2.Post("/auth/signup", ctrl.SignUpUser)
    r2, _ := http.NewRequest("POST", "/auth/signup", bytes.NewReader([]byte(`{}`)))
    r2.Header.Set("Content-Type", "application/json")
    resp2, err := app2.Test(r2)
    if err != nil { t.Fatalf("request failed: %v", err) }
    if resp2.StatusCode != 400 { t.Fatalf("expected 400 got %d", resp2.StatusCode) }
...
func TestLogoutUser(t *testing.T) {
    app := fiber.New()
    ctrl := &AuthController{}
    app.Post("/auth/logout", ctrl.LogoutUser)
